function [Qsim, SM, SUZ, SLZ, SNOWPACK, ETact, MELTWATER] = HBV_lumpedx(P,ETpot,T,parameters)
    %{
    Qsim = HBV_lumped(P,ETpot,T,parameters)
    [Qsim, SM, SUZ, SLZ, SNOWPACK, ETact, MELTWATER] = HBV_lumped(P,ETpot,T,parameters)
    ...
    Last modified:
       ...
       Mar 2021: Add MELTWATER to the list of output (Natt)
    %}
    if nargin~=4
    ...
    %     fprintf('Spinup1 by %2.0f\n',size(P,1));
    else
        % add 10 years period to the front, then use the last day as initial
        % state of the original period
        init_period = int32(10*365.25);
        P = [P(1:init_period,:); P];
        ETpot = [ETpot(1:init_period,:); ETpot];
        T = [T(1:init_period,:); T];
    %     fprintf('Spinup2 by %2.0f\n',size(P,1));
    end
